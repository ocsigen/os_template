           ...
           Lwt_js_events.clicks (Eliom_content.Html.To_dom.of_element ~%button)
             (fun _ _ ->
               ...
               Eliom_client.change_page
                 ~service:Project_name_services.settings_service () ()))
        : unit)];
  button
...
let%shared buttons_of_email is_main_email is_validated email =
  if is_main_email then []
  else if is_validated then
    [ update_main_email_button email; delete_email_button email ]
  ...
  let valid_label =
    span
      ...
      [
        (txt
        @@
        if is_validated then [%i18n S.validated ~capitalize:true]
...
let%shared select_language_form select_language_name =
  let open Eliom_content.Html in
  let current_language = Project_name_i18n.get_language () in
  let all_languages_except_current =
    List.filter (fun l -> l <> current_language) Project_name_i18n.languages
  in
  let form_option_of_language language is_current_language =
    D.Form.Option
      ( [],
        (* No attributes *)
        Project_name_i18n.string_of_language language,
        None,
        is_current_language )
  in
  [
         ...
         div
           ...
           [
             ...
             Os_user_view.upload_pic_link
               ...
               Project_name_services.upload_user_avatar_service;
             br ();
