...
let%server add_email_notif () =
  if Eliom_reference.Volatile.get Os_user.user_already_exists then
    Os_msg.msg ~level:`Err ~onload:true [%i18n S.email_already_exists]
...
*)
let%server _ =
  if Eliom_config.get_debugmode () then (
    ignore
      [%client
