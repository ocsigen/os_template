    ...
    Ot_spinner.with_spinner
      (let* users = get_users () in
       let users =
         List.map
           (fun u ->
             if u = "" then li [ em [ txt "new user" ] ] else li [ txt u ])
           users
       in
       if users = [] then
         Lwt.return [ p [ em [%i18n Demo.no_user_create_accounts] ] ]
...
let%shared () =
  Project_name_base.App.register ~service:Demo_services.demo_pgocaml
    ( Project_name_page.Opt.connected_page @@ fun myid_o () () ->
      let* p = page () in
      Project_name_container.page
        ~a:[ a_class [ "os-page-demo-pgocaml" ] ]
        myid_o p )
